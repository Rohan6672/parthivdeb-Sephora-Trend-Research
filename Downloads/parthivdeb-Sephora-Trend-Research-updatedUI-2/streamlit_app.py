...
def init_state() -> None:
    ...
    if "current_view" not in st.session_state:
        st.session_state["current_view"] = "Sephora Trends Dashboard"
    if "rate_limit_until" not in st.session_state:
        st.session_state["rate_limit_until"] = None
    if "score_proportions" not in st.session_state:
        st.session_state["score_proportions"] = {"titles": 45, "views": 25, "shares": 8}


@st.dialog("Update Score Proportions")
def show_update_score_dialog():
    """Display dialog for updating score proportions"""
    proportions = st.session_state.get("score_proportions", {"titles": 45, "views": 25, "shares": 8})

    st.markdown("### Titles (%)")
    titles = st.number_input(
        "Titles percentage",
        min_value=0,
        max_value=100,
        value=proportions["titles"],
        key="dialog_titles",
        label_visibility="collapsed"
    )
    st.caption("Reference: Allure, Byrdie")
    ...
    views = st.number_input(
        ...
        min_value=0,
        max_value=100,
        value=proportions["views"],
        key="dialog_views",
        label_visibility="collapsed"
    )
    st.caption("Reference: YouTube, TikTok")
    ...
    shares = st.number_input(
        ...
        min_value=0,
        max_value=100,
        value=proportions["shares"],
        key="dialog_shares",
        label_visibility="collapsed"
    )
    st.caption("Reference: Instagram")

    st.markdown("")
    total = titles + views + shares
    st.markdown(f"**Total: {total}%**")

    st.markdown("")

    if st.button("Save Proportions", use_container_width=True, type="primary"):
        st.session_state["score_proportions"] = {
            "titles": titles,
            "views": views,
            "shares": shares
        }
        st.success(f"Proportions saved! Total: {total}%")
        st.rerun()
...
def render_trend_card_compact(trend: Trend) -> None:
    ...
    if st.button("View Full Details", key=f"detail_{trend['id']}", use_container_width=True):
        ...
        # Store which view we're coming from
        st.session_state["previous_view"] = st.session_state.get("current_view", "Sephora Trends Dashboard")
        st.rerun()
...
def render_full_detail_view(trend: Trend) -> None:
    ...
    with col2:
        if st.button("✕", key="close_detail"):
            st.session_state["detail_view_trend"] = None
            # If came from Trend Discovery, clear the search query
            if st.session_state.get("previous_view") == "Trend Discovery":
                st.session_state["pending_query"] = ""
                st.session_state["search_result"] = None
            # Don't clear previous_view here - let main() handle it
            st.rerun()
    ...
    with col1:
        st.markdown(
            f"""
            <div style="background: #f8f9fa; padding: 20px 20px 10px 20px; border-radius: 8px; height: 120px; display: flex; flex-direction: column;">
                <div style="color: #888; font-size: 12px;">📈 Virality</div>
                <div style="color: #ff69b4; font-size: 32px; font-weight: 700; margin-top: 10px; flex-grow: 1;">{trend['viralityScore']}</div>
            </div>
            """,
            unsafe_allow_html=True
        )
        if st.button("Update ⚙️", key=f"update_virality_{trend['id']}", use_container_width=True):
            show_update_score_dialog()
    with col2:
        st.markdown(
            f"""
            <div style="background: #f8f9fa; padding: 20px; border-radius: 8px; height: 120px; display: flex; flex-direction: column;">
                <div style="color: #888; font-size: 12px;">🌍 World</div>
                <div style="font-size: 32px; font-weight: 700; margin-top: 10px; color: #000; flex-grow: 1;">{trend.get('world', 'N/A')}</div>
                <div style="color: transparent; font-size: 12px;">placeholder</div>
            </div>
            """,
            unsafe_allow_html=True
        )
    ...
    with col1:
        st.markdown("**EXTERNAL BEAUTY EXPERT COMMENTS**")
    ...
    # Citations expander
    with st.expander("ℹ️ View Citations"):
        st.caption("Vogue Beauty - Expert Analysis: Spring Beauty Trends")
        st.caption("https://www.vogue.com/article/spring-beauty-trends-expert-analysis")
        st.caption("")
        st.caption("Allure Magazine - Dr. Sarah Chen on Emerging Trends")
        st.caption("https://www.allure.com/story/beauty-expert-sarah-chen-interview")
        st.caption("")
        st.caption("WWD Beauty Summit 2024 - Panel Discussion Transcript")
        st.caption("https://wwd.com/beauty-industry-news/beauty-summit-2024-transcript")
    ...
    with tab5:
        ...
        # Define category-subcategory mapping
        CATEGORY_SUBCATEGORIES = {
            "Clean": [],
            "Cleansers": ["Blotting Papers", "Exfoliators", "Face Wash & Cleansers", "Face Wipes", "Makeup Removers", "Toners"],
            "Eye Care": ["Eye Creams & Treatments", "Eye Masks"],
            "High Tech Tools": ["Anti-Aging", "Facial Cleansing Brushes", "Hair Removal", "Teeth Whitening"],
            "Lip Balms & Treatments": [],
            "Masks": ["Face Masks", "Sheet Masks"],
            "Mini Size": [],
            "Moisturizers": ["BB & CC Creams", "Decollete & Neck Creams", "Face Oils", "Mists & Essences", "Moisturizers", "Night Creams"],
            "Self Tanners": ["For Body", "For Face"],
            "Sunscreen": ["Body Sunscreen", "Face Sunscreen"],
            "Treatments": ["Blemish & Acne Treatments", "Face Serums", "Facial Peels"],
            "Value & Gift Sets": [],
            "Wellness": ["Beauty Supplements", "Facial Rollers", "Holistic Wellness"]
        }
        ...
        with col1:
            selected_category = st.selectbox(
                "Select Category",
                options=list(CATEGORY_SUBCATEGORIES.keys()),
                key=f"performance_category_{trend['id']}"
            )

        with col2:
            # Subcategory dropdown based on selected category
            subcategories = CATEGORY_SUBCATEGORIES.get(selected_category, [])
            if subcategories:
                selected_subcategory = st.selectbox(
                    "Select Sub-Category",
                    options=subcategories,
                    key=f"performance_subcategory_{trend['id']}"
                )
            else:
                st.selectbox(
                    "Select Sub-Category",
                    options=["No subcategories available"],
                    disabled=True,
                    key=f"performance_subcategory_disabled_{trend['id']}"
                )
...
def main() -> None:
    ...
    if st.session_state.get("detail_view_trend") is not None:
        ...
        # Use previous_view if just returned from detail view, otherwise use radio button
        if st.session_state.get("previous_view") is not None:
            # Just returned from detail view, use stored view
            view = st.session_state["previous_view"]
            st.session_state["previous_view"] = None  # Clear it after using
            st.session_state["current_view"] = view

        view_options = ("Sephora Trends Dashboard", "Trend Discovery")
        current_index = view_options.index(st.session_state.get("current_view", "Sephora Trends Dashboard"))

        view = st.sidebar.radio("View", view_options, index=current_index, key="view_selector")
